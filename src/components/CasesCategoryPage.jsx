...
const categoryConfig = {
  ...
  '/cases/medicine': {
    ...
    cases: [
      ...
      { title: 'Авиандр', placeholder: 'А', note: 'Медицинская анимация, доказательная база, маскоты и визуальная система препарата.' },
    ],
  },
  '/cases/cinema': {
  ...
  '/cases/hse': {
    ...
    icon: HardHat,
    cases: [
      { title: 'Мултон Партнерс', image: multonImage, note: 'Маскот и визуальная система коммуникации жизненно важных правил.', href: '/cases/multon-partners/' },
    ],
  },
};
...
function CategoryCaseCard({ item }) {
  ...
  return (
    <a className="cases-hub-card" href={item.href} target={external ? '_blank' : undefined} rel={external ? 'noreferrer' : undefined}>
      {content}
    </a>
  );
}
  ...
  return (
    <main className="cases-hub-page cases-category-page">
      <header className="cases-hub-header">
        ...
        <a className="cases-hub-header__cta" href="https://t.me/anix_helper" target="_blank" rel="noreferrer">Обсудить проект</a>
      </header>
      ...
      <section className="cases-hub-category cases-category-listing">
        <div className={`cases-hub-grid${category.cases.length === 1 ? ' cases-hub-grid--single' : ''}`}>
          {category.cases.map((item) => <CategoryCaseCard item={item} key={item.title} />)}
        </div>
      </section>
      ...
      <section className="cases-hub-cta">
        ...
        <a href="/cases/">Открыть все кейсы <ArrowRight aria-hidden="true" /></a>
      </section>
