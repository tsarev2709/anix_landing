...
const categories = [
  {
    id: 'b2b', eyebrow: 'B2B / Technology', title: 'Технологии, B2B и сложные продукты',
    description: 'Когда продукт сложно объяснить одним слайдом: превращаем механику, пользу и контекст в историю, которую быстрее понимают клиенты, партнёры и инвесторы.', icon: Workflow,
    cases: [
  ...
  {
    id: 'medicine', eyebrow: 'Pharma / MedTech', title: 'Pharma, MedTech и медицина',
    description: 'Работаем там, где одновременно важны точность, доверие и внимание: медицинские продукты, препараты, научная логика, врачи и пациенты.', icon: Stethoscope,
    cases: [
  ...
  {
    id: 'cinema', eyebrow: 'Cinema / Creative', title: 'Cinema & Creative',
  ...
  {
    id: 'hse', eyebrow: 'HSE / Safety', title: 'Охрана труда',
    description: 'Визуальные системы, которые помогают правилам безопасности не растворяться в регламентах и действительно оставаться в поле внимания сотрудников.', icon: HardHat,
    cases: [{ title: 'Мултон Партнерс', image: multonImage, note: 'Маскот и визуальная система коммуникации жизненно важных правил.', href: '/cases/multon-partners/', featured: true }],
  },
];
...
export default function CasesHubPage() {
  return <main className="cases-hub-page">
    ...
    <section className="cases-hub-hero"><p className="cases-hub-eyebrow">Кейсы Anix Studio</p><h1>Сложные продукты, которые стали понятными историями</h1><p>Здесь собраны проекты для технологий, B2B, фармы, MedTech, кино и охраны труда. Разные задачи и визуальные языки — один принцип: сначала понять, что должно произойти со зрителем.</p><nav className="cases-hub-jump" aria-label="Категории кейсов">{categories.map((category) => <a href={`/cases/${category.id}/`} key={category.id}>{category.title}</a>)}<a href="#events">Events</a></nav></section>
    {categories.map((category) => { const Icon = category.icon; return <section className="cases-hub-category" id={category.id} key={category.id}><div className="cases-hub-category__head"><div className="cases-hub-category__icon"><Icon aria-hidden="true" /></div><div><p className="cases-hub-eyebrow">{category.eyebrow}</p><h2>{category.title}</h2><p>{category.description}</p><a href={`/cases/${category.id}/`}>Открыть категорию <ArrowRight aria-hidden="true" /></a></div></div><div className={`cases-hub-grid${category.cases.length === 1 ? ' cases-hub-grid--single' : ''}`}>{category.cases.map((item) => <CaseCard item={item} key={item.title} />)}</div></section>; })}
    <section className="cases-hub-events" id="events"><div className="cases-hub-events__icon"><Sparkles aria-hidden="true" /></div><p className="cases-hub-eyebrow">Events</p><h2>Скоро</h2><p>Добавим проекты с экранным контентом, AI-визуалами и режиссурой событий.</p></section>
    <section className="cases-hub-cta"><p className="cases-hub-eyebrow">Новая задача</p><h2>Не нашли кейс ровно из вашей отрасли?</h2><p>Покажите продукт или идею. Разберёмся в задаче и предложим формат, который имеет смысл именно для неё.</p><a href="https://t.me/anix_helper" target="_blank" rel="noreferrer">Обсудить проект <ArrowRight aria-hidden="true" /></a></section>
    <SiteFooter />
  </main>;
}