...
async function main() {
  if (!commandExists('pdftoppm')) {
    throw new Error('pdftoppm is required. Install poppler-utils before running the production build.');
  }
  ...
  for (let index = 1; index <= 10; index += 1) {
    ...
    const png = `${tempPrefix}-1.png`;

    if (!fs.existsSync(pdf)) throw new Error(`Missing Rybki PDF: ${pdf}`);

    const render = spawnSync('pdftoppm', ['-f', '1', '-singlefile', '-png', '-r', '180', pdf, tempPrefix], {
      stdio: 'inherit',
    });
    if (render.status !== 0 || !fs.existsSync(png)) {
      throw new Error(`Failed to render ${path.basename(pdf)}`);
    }
    ...
    for (const width of widths) {
      ...
      await sharp(png)
        ...
        .toFile(output);
      console.log(`[rybki-pdf] ${path.basename(output)} ${(fs.statSync(output).size / 1024).toFixed(1)} KiB`);
    }
