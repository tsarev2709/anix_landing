...
function waitForServer(timeoutMs = 15000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const retry = () => {
      if (Date.now() - startedAt > timeoutMs) { reject(new Error(`Static server was not ready in ${timeoutMs}ms`)); return; }
...
async function main() {
  ...
  try {
    ...
    capture(chromePath, 'desktop-gallery', `${baseUrl}/andrey-tsarev/#gallery`, 1440, 1600);
    capture(chromePath, 'mobile-hero', `${baseUrl}/andrey-tsarev/`, 390, 1500);
    capture(chromePath, 'mobile-gallery', `${baseUrl}/andrey-tsarev/#gallery`, 390, 1500);
  } finally {
    server.kill('SIGTERM');
  }
